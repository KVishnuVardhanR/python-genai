...
def require_agent_name(context: Any) -> str:
  ...
  if hasattr(context, 'agent') and hasattr(context.agent, 'name') and context.agent.name:
...
class AudioCacheManager:
  ...
  async def flush_caches(
    ...
    if flush_user_audio and getattr(invocation_context, 'input_realtime_cache', None):
    ...
    if flush_model_audio and getattr(invocation_context, 'output_realtime_cache', None):
  ...
  async def _flush_cache_to_services(
    ...
    """
    if not getattr(invocation_context, 'artifact_service', None) or not audio_cache:
    ...
    try:
      ...
      session_id = (
          getattr(getattr(invocation_context, 'session', None), 'id', None)
  ...
  def get_cache_stats(
    ...
    """
    input_chunks = getattr(invocation_context, 'input_realtime_cache', None) or []
    output_chunks = getattr(invocation_context, 'output_realtime_cache', None) or []
    ...
    input_bytes = sum(
        ...
        for entry in input_chunks
    )
    output_bytes = sum(
        len(_require_audio_data(entry.data))
        for entry in output_chunks
    )
