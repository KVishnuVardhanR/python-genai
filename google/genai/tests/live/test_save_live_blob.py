...
async def test_async_session_records_input_and_output_audio(tmp_path):
  ...
  artifacts_dir = tmp_path / "users" / "test_user" / "sessions" / "test_live_session" / "artifacts"
  saved_input_files = list(artifacts_dir.glob("live_audio_storage_input_audio_*"))
  saved_output_files = list(artifacts_dir.glob("live_audio_storage_output_audio_*"))
  ...
  saved_output_payload = (output_artifact_dir / "versions" / "0" / output_artifact_dir.name).read_bytes()
