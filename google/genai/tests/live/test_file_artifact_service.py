...
async def test_save_artifact_creates_version_and_atomic_metadata(temp_service):
  ...
  assert meta.get("fileName") == "test_audio.pcm" or meta.get("file_name") == "test_audio.pcm"
  assert meta["version"] == 0
  assert meta.get("mimeType") == "audio/pcm" or meta.get("mime_type") == "audio/pcm"
  assert meta.get("customMetadata") == {"key": "val"} or meta.get("custom_metadata") == {"key": "val"}
...
async def test_multiple_versions(temp_service):
  part1 = types.Part(inline_data=types.Blob(data=b"v1_data", mime_type="audio/pcm"))
  part2 = types.Part(inline_data=types.Blob(data=b"v2_data", mime_type="audio/pcm"))
...
async def test_delete_artifact(temp_service):
  ...
  await temp_service.save_artifact(
      app_name="app", user_id="u", session_id="s", filename="del.pcm", artifact=part
  )
