...
async def test_multiple_audio_caching(manager):
  ...
  for i in range(3):
    manager.cache_audio(context, types.Blob(data=f'in_{i}'.encode(), mime_type='audio/pcm'), 'input')
  for i in range(2):
    manager.cache_audio(context, types.Blob(data=f'out_{i}'.encode(), mime_type='audio/wav'), 'output')
...
async def test_flush_caches_both(manager):
  ...
  manager.cache_audio(context, types.Blob(data=b'input_data', mime_type='audio/pcm'), 'input')
  manager.cache_audio(context, types.Blob(data=b'output_data', mime_type='audio/wav'), 'output')
...
async def test_flush_caches_selective(manager):
  ...
  events = await manager.flush_caches(context, flush_user_audio=True, flush_model_audio=False)
  assert len(events) == 1
  assert context.input_realtime_cache == []
...
async def test_get_cache_stats_with_data(manager):
  ...
  input_blob2 = types.Blob(data=b'1234567890', mime_type='audio/pcm')  # 10 bytes
...
async def test_error_handling_in_flush(manager):
  ...
  mock_artifact_service.save_artifact.side_effect = Exception('Artifact service error')
...
async def test_filename_uses_first_chunk_timestamp(manager):
  ...
  assert filename == f"live_audio_storage_input_audio_{expected_timestamp_ms}.pcm;rate=16000"
...
def test_invalid_cache_type(manager):
  ...
  with pytest.raises(ValueError, match="cache_type must be either 'input' or 'output'"):
...
async def test_output_audio_mime_type_defaults_to_24000_rate(manager):
  ...
  events = await manager.flush_caches(context, flush_user_audio=False, flush_model_audio=True)
  assert len(events) == 1
  mock_artifact_service.save_artifact.assert_called_once()
  saved_artifact = mock_artifact_service.save_artifact.call_args.kwargs['artifact']
  assert saved_artifact.inline_data.mime_type == 'audio/pcm;rate=24000'
  assert events[0].content.parts[0].file_data.mime_type == 'audio/pcm;rate=24000'
...
async def test_input_audio_mime_type_defaults_to_16000_rate(manager):
  ...
  assert events[0].content.parts[0].file_data.mime_type == 'audio/pcm;rate=16000'
