...
class AsyncSession:
  ...
  async def send_realtime_input(
    ...
    if self.save_live_blob and self.audio_cache_manager:
      if media is not None:
        if isinstance(media, types.Blob) and media.mime_type and media.mime_type.startswith('audio/'):
          self.audio_cache_manager.cache_audio(self, media, cache_type='input')
        elif isinstance(media, dict) and bool(media.get('mime_type') and str(media.get('mime_type')).startswith('audio/')):
          self.audio_cache_manager.cache_audio(self, types.Blob(data=media.get('data', b''), mime_type=media.get('mime_type')), cache_type='input')
      if audio is not None:
        if isinstance(audio, types.Blob):
          self.audio_cache_manager.cache_audio(self, audio, cache_type='input')
        elif isinstance(audio, dict):
          self.audio_cache_manager.cache_audio(self, types.Blob(data=audio.get('data', b''), mime_type=audio.get('mime_type', 'audio/pcm')), cache_type='input')
  ...
  async def _receive(self) -> types.LiveServerMessage:
    ...
    if self.save_live_blob and self.audio_cache_manager:
      if result.server_content and result.server_content.model_turn and result.server_content.model_turn.parts:
        for part in result.server_content.model_turn.parts:
          if part.inline_data and part.inline_data.mime_type and part.inline_data.mime_type.startswith('audio/'):
            audio_blob = types.Blob(data=part.inline_data.data, mime_type=part.inline_data.mime_type)
            self.audio_cache_manager.cache_audio(self, audio_blob, cache_type='output')
      if result.server_content and result.server_content.interrupted:
        await self.audio_cache_manager.flush_caches(self, flush_user_audio=False, flush_model_audio=True)
      elif result.server_content and result.server_content.turn_complete:
        await self.audio_cache_manager.flush_caches(self, flush_user_audio=True, flush_model_audio=True)
    return result
  ...
  async def connect(
    ...
    async with ws_connect(
      ...
      yield AsyncSession(
          ...
          save_live_blob=bool(parameter_model.save_live_blob),
      )
